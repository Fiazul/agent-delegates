...
function workerPreamble(vendor) {
  ...
  const label = { codex: 'Codex', agy: 'Antigravity', grok: 'Grok', claude: 'Claude' }[vendor];
...
async function invoke(mode, vendorInput, tierOrId, briefSource, options = {}, stdinText = '') {
  ...
  if (!['codex', 'agy', 'grok', 'claude'].includes(vendor)) throw new Error(`unknown vendor '${vendorInput}'`);
  ...
  const commands = { codex: 'codex', agy: 'agy', grok: 'grok', claude: 'claude' };
  ...
  const idField = { codex: 'thread_id', agy: 'conversation_id', grok: 'session_id', claude: 'session_id' }[vendor];
  ...
  const open = vendor === 'codex' ? `CODEX_HOME=${job.env.CODEX_HOME} codex resume ${result.id}`
    : vendor === 'agy' ? `agy --conversation ${result.id}`
      : vendor === 'grok' ? `grok --resume ${result.id}` : `claude --resume ${result.id}`;
