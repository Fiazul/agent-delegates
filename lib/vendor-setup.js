...
function displayedCommand(vendor, platform) {
  ...
  if (platform === 'win32' && vendor.id === 'grok') return `Manual install: ${vendor.windowsManual}`;
...
async function defaultInstall(vendor, platform) {
  ...
  if (platform === 'win32') {
    if (vendor.id === 'grok') throw new Error(`Grok CLI for Windows has no verified installer URL. See ${vendor.windowsManual}`);
...
async function setupVendorClis(options = {}) {
  ...
  for (const vendor of VENDORS) {
    ...
    if (platform === 'win32' && vendor.id === 'grok') {
      log(`Grok CLI for Windows needs manual installation. See ${vendor.windowsManual}`);
      result.skipped.push(vendor.id);
      continue;
    }
