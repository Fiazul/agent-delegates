...
const { exists, homeDir, installHome, packageRoot, readJson, shellQuote, timestamp, writeJson } = require('./util');
...
const MAIN_SKILL_DIR = Object.freeze({ claude: '.claude', cursor: '.cursor' });
// Legacy skill dirs from before skill targets were verified against each orchestrator CLI's own
// loader (2026-09-18 audit): codex used to link into `~/.agents/skills`, but the installed
// `codex` binary's own --help says it loads skills from `$CODEX_HOME/skills` (default
// `~/.codex/skills`) — `.agents/skills` is an unrelated dir codex never reads. Kept here only so
// `--uninstall` can still sweep a prior wrong-location install; never a link target going
// forward.
const LEGACY_SKILL_DIRS = Object.freeze([path.join('.agents', 'skills')]);

// Where each orchestrator CLI actually loads skills from — verified against the real installed
// binaries, not assumed from convention (2026-09-18):
//  - codex: `$CODEX_HOME/skills`, default `~/.codex/skills` when CODEX_HOME is unset. Confirmed
//    via `codex --help`: "create discoverable skills in `$CODEX_HOME/skills`" / "Installs into
//    `$CODEX_HOME/skills/<skill-name>`". This is the orchestrator's own CODEX_HOME, unrelated to
//    the `~/.codex-fresh` clean-room default used for Codex *workers* elsewhere in this project.
//  - cursor: `~/.cursor/skills`. Confirmed via `strings` on cursor-agent's bundled index.js,
//    which references `.cursor/skills/`, `~/.cursor/skills/`, and `.cursor/skills-cursor/...`
//    literally — no separate global/home override path found.
//  - claude: `~/.claude/skills`. Known-good (this tool itself runs on it).
function skillDirFor(main, env, home) {
  if (main === 'codex') {
    const codexHome = (env && env.CODEX_HOME) || path.join(home, '.codex');
    return path.join(codexHome, 'skills');
  }
  return path.join(home, MAIN_SKILL_DIR[main], 'skills');
}
...
// Only ever removes a symlink whose target actually resolves inside our own copied package dir
// (pkgDir(home)) — a same-named symlink some other tool created at the same path (e.g. a user's
// own `delegates` skill link) is never touched, even during the legacy-dir sweep.
function removeManagedLink(target, home, log) {
  let stat;
  try { stat = fs.lstatSync(target); } catch { return; }
  if (!stat.isSymbolicLink()) return;
  let linkTarget;
  try { linkTarget = fs.readlinkSync(target); } catch { return; }
  const resolved = path.resolve(path.dirname(target), linkTarget);
  const pkg = pkgDir(home);
  if (resolved !== pkg && !resolved.startsWith(pkg + path.sep)) return;
...
// H3: everything an install leaves behind used to point straight at packageRoot() — the directory
// the currently-running code happens to live in. Under `npx --yes github:...`, that's an ephemeral
// cache directory npx can prune at any time, silently breaking every symlink/shim/hook copy that
// pointed at it. Fix: copy the package's own shipped files (per package.json's `files` list, plus
// package.json itself) into a stable, non-transient install home first, then have every artifact
// this module writes reference THAT copy instead. Fresh-replace semantics (old copy removed, then
// recopied) so re-running `install` — e.g. after the real package updated — always reflects the
// current code. No-ops when we're already running from inside the install home (a global npm
// install there, or a re-run via the installed shim itself), since there is nothing to copy onto
// itself.
const PACKAGE_FILES_FALLBACK = Object.freeze(['bin', 'lib', 'skills', 'extras', 'README.md', 'LICENSE']);

function packageManifestEntries() {
  const pkgJson = readJson(path.join(packageRoot(), 'package.json'), null);
  const files = pkgJson && Array.isArray(pkgJson.files) ? pkgJson.files : PACKAGE_FILES_FALLBACK;
  return files.map(f => f.replace(/\/$/, ''));
}

function pkgDir(home) {
  return path.join(installHome(home), 'pkg');
}

function copyPackageToInstallHome(home, log) {
  const src = packageRoot();
  const dest = pkgDir(home);
  if (path.resolve(src) === path.resolve(dest)) return dest; // already running from the install home

  fs.rmSync(dest, { recursive: true, force: true });
  fs.mkdirSync(dest, { recursive: true });
  for (const entry of packageManifestEntries()) {
    const from = path.join(src, entry);
    if (!exists(from)) continue;
    fs.cpSync(from, path.join(dest, entry), { recursive: true });
  }
  const pkgJsonFrom = path.join(src, 'package.json');
  if (exists(pkgJsonFrom)) fs.cpSync(pkgJsonFrom, path.join(dest, 'package.json'));
  log(`package copied to ${dest}`);
  return dest;
}

// Command shims: a plain executable script/batch file on a stable PATH directory that execs
// `<installed cli.js> "$@"` — never an alias, since bash/zsh aliases don't exist in the
// non-interactive shells most agent tooling (including this one) spawns. Every shim we write
// carries SHIM_MARKER so a later install/uninstall can tell "ours, safe to overwrite/remove" apart
// from some unrelated file a user or another tool happens to have at the same path — we must never
// clobber or delete a foreign file there.
const SHIM_MARKER = 'agent-delegates shim';
const POSIX_SHIM_NAMES = Object.freeze(['agent-delegates', 'delegates']);
const WIN32_SHIM_NAMES = Object.freeze(['agent-delegates.cmd', 'delegates.cmd']);

function isOurShim(file) {
  if (!exists(file)) return true; // nothing there yet: trivially safe to write
  try {
    return fs.readFileSync(file, 'utf8').includes(SHIM_MARKER);
  } catch {
    return false; // unreadable (e.g. a directory) — treat as foreign, never touch it
  }
}

function posixBinDir(home) {
  return path.join(home, '.local', 'bin');
}

function posixShimContent(cliPath) {
  return `#!/bin/sh\n# ${SHIM_MARKER}\nexec ${shellQuote(process.execPath)} ${shellQuote(cliPath)} "$@"\n`;
}

function win32ShimContent(cliPath) {
  return `@echo off\r\n@rem ${SHIM_MARKER}\r\n${shellQuote(process.execPath)} ${shellQuote(cliPath)} %*\r\n`;
}

function isDirOnPath(dir, env) {
  const raw = (env && (env.PATH ?? env.Path)) || '';
  const resolved = path.resolve(dir);
  return raw.split(path.delimiter).some(p => p && path.resolve(p) === resolved);
}

function writeShimSet(binDir, names, content, chmod, log) {
  ...
  for (const name of names) {
    ...
    if (chmod) { try { fs.chmodSync(target, 0o755); } catch {} }
    written.push(target);
  }
  for (const target of skipped) log(`WARNING: ${target} already exists and is not an agent-delegates shim — left it alone`);
  return { written, skipped, binDir };
}

function installPosixShims(home, pkgRoot, options, log) {
  const env = options.env || process.env;
  const binDir = posixBinDir(home);
  const cliPath = path.join(pkgRoot, 'bin', 'cli.js');
  const result = writeShimSet(binDir, POSIX_SHIM_NAMES, posixShimContent(cliPath), true, log);
  const onPath = isDirOnPath(binDir, env);
  if (result.written.length) log(`command shim installed: ${result.written.join(', ')}`);
  if (!onPath) {
    log(`NOTE: ${binDir} is not on your PATH. Add this to ~/.bashrc or ~/.zshrc, then restart your shell:`);
    log(`  export PATH="${binDir}:$PATH"`);
  }
  return { ...result, onPath };
}

function applyWin32Path(binDir, options, log) {
  try {
    const spawnSetx = options.spawnSetx || ((args) => require('node:child_process').spawnSync('setx', args, { stdio: 'ignore' }));
    const env = options.env || process.env;
    const current = env.Path ?? env.PATH ?? '';
    const outcome = spawnSetx(['PATH', `${current};${binDir}`]);
    if (outcome && outcome.status === 0) log(`PATH updated via setx (open a new terminal for it to take effect): ${binDir}`);
    else log(`could not update PATH automatically; add manually: setx PATH "%PATH%;${binDir}"`);
  } catch (error) {
    log(`could not update PATH automatically (${error.message}); add manually: setx PATH "%PATH%;${binDir}"`);
  }
}

async function installWin32Shims(home, pkgRoot, options, isTTY, promptText, log) {
  const env = options.env || process.env;
  const binDir = path.join(installHome(home), 'bin');
  const cliPath = path.join(pkgRoot, 'bin', 'cli.js');
  const result = writeShimSet(binDir, WIN32_SHIM_NAMES, win32ShimContent(cliPath), false, log);
  if (result.written.length) log(`command shim installed: ${result.written.join(', ')}`);
  let onPath = isDirOnPath(binDir, env);
  if (!onPath) {
    if (options.noPath) {
      log(`NOTE: ${binDir} is not on your PATH. Add it manually:`);
      log(`  setx PATH "%PATH%;${binDir}"`);
    } else if (isTTY) {
      const answer = await promptText(`Add ${binDir} to your user PATH? [Y/n] `);
      const yes = !/^n(o)?$/i.test(String(answer).trim());
      if (yes) { applyWin32Path(binDir, options, log); onPath = true; }
      else {
        log(`NOTE: ${binDir} is not on your PATH. Add it manually:`);
        log(`  setx PATH "%PATH%;${binDir}"`);
      }
    } else {
      log(`NOTE: ${binDir} is not on your PATH. Add it manually (or re-run interactively to be asked):`);
      log(`  setx PATH "%PATH%;${binDir}"`);
    }
  }
  return { ...result, onPath };
...
// Never touches the machine-wide PATH (win32's setx /M) — only the current user's, and only with
// explicit consent (a TTY prompt, since --yes governs vendor CLI installs, not this).
async function installCommandShims(home, pkgRoot, options, isTTY, promptText, log) {
  const shimResult = process.platform === 'win32'
    ? await installWin32Shims(home, pkgRoot, options, isTTY, promptText, log)
    : installPosixShims(home, pkgRoot, options, log);

  if (shimResult.written.length && shimResult.onPath) {
    log(`Command available: agent-delegates (shim at ${shimResult.written[0]})`);
  } else if (shimResult.written.length) {
    log(`Command available: agent-delegates (shim at ${shimResult.written[0]}) once ${shimResult.binDir} is on your PATH — see note above`);
  } else {
    const cliPath = path.join(pkgRoot, 'bin', 'cli.js');
    log(`Command not installed (a foreign file occupies its path) — run it directly instead: "${process.execPath}" "${cliPath}"`);
  }
  return shimResult;
}

function removeCommandShims(home, log) {
  const specs = process.platform === 'win32'
    ? [[path.join(installHome(home), 'bin'), WIN32_SHIM_NAMES]]
    : [[posixBinDir(home), POSIX_SHIM_NAMES]];
  for (const [binDir, names] of specs) {
    for (const name of names) {
      const target = path.join(binDir, name);
      if (!exists(target)) continue;
      if (!isOurShim(target)) { log(`left ${target} alone (not an agent-delegates shim)`); continue; }
      fs.unlinkSync(target);
      log(`removed ${target}`);
    }
  }
...
function installNudgeCopy(home, pkgRoot = packageRoot()) {
  ...
  for (const [src, destParts] of NUDGE_COPY_FILES) {
    ...
    fs.copyFileSync(path.join(pkgRoot, src), dest);
  }
...
function installHook(home, log, pkgRoot = packageRoot()) {
  const claudeDir = path.join(home, '.claude');
  const settingsFile = path.join(claudeDir, 'settings.json');
  ...
  fs.mkdirSync(claudeDir, { recursive: true });
  const implPath = installNudgeCopy(home, pkgRoot);
...
async function install(options = {}) {
  ...
  const env = options.env || process.env;
  const allTargets = [skillDirFor('claude', env, home), skillDirFor('codex', env, home), skillDirFor('cursor', env, home)];
  const ts = timestamp();

  if (options.uninstall) {
    // Uninstall always sweeps all three current skill dirs (we don't track which --main was used
    // to install) plus any legacy wrong-location dirs from before a target was corrected (e.g.
    // codex's old `~/.agents/skills`), so a pre-fix install still gets fully cleaned up. Never
    // touches routing.json — that file is the user's own config.
    const legacyTargets = LEGACY_SKILL_DIRS.map(rel => path.join(home, rel));
    for (const root of [...allTargets, ...legacyTargets]) for (const skill of SKILLS) removeManagedLink(path.join(root, skill), home, log);
    appendAlias(home, true, log);
    removeBinAliases(home, log);
    removeCommandShims(home, log);
    uninstallHook(home, log);
    const installedPkgDir = pkgDir(home);
    if (exists(installedPkgDir)) {
      fs.rmSync(installedPkgDir, { recursive: true, force: true });
      log(`removed ${installedPkgDir}`);
    }
    log('aliases removed');
    return;
  }

  // H3: copy the running package's own files into a stable install home first — every artifact
  // below (skill symlinks, statusline/hook copy sources, the command shims) references pkgRoot,
  // never packageRoot() directly, so nothing installed ever points back at a transient `npx`
  // cache. No-op (returns packageRoot() unchanged) when already running from inside that home.
  const pkgRoot = copyPackageToInstallHome(home, log);
  const sourceRoot = path.join(pkgRoot, 'skills');
  ...
  const targets = main === 'all' ? allTargets : [skillDirFor(main, env, home)];
  ...
  if (main === 'claude' || main === 'all') {
    if (!options.noStatusline) installStatusline(home, log, pkgRoot);
    if (!options.noHook) installHook(home, log, pkgRoot);
  } else {
  ...
  await installCommandShims(home, pkgRoot, options, isTTY, promptText, log);

  log('Skills configured. Check CLI availability with: agent-delegates status');
}

module.exports = {
  SKILLS, hookStub, install, installHook, installStatusline, uninstallHook,
  copyPackageToInstallHome, pkgDir, installCommandShims, removeCommandShims, isOurShim, posixBinDir,
};
