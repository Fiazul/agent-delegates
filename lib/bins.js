...
// assessAgentConflict({ env }) -> { hits, effective, cursorInstalled, conflict }
//
// The category this fixes: "conflict" used to mean "two owners exist somewhere on PATH", which
// fired even when the shell's actual `agent` resolved correctly (e.g. Cursor's `agent` first,
// Grok's shadowed duplicate harmlessly further down). The only thing a user can actually be hurt
// by is the EFFECTIVE `agent` — the first hit PATH resolution would pick — being wrong. So:
//   - `hits` is detectAgentCollision's ordered (PATH-order) list, unchanged.
//   - `effective` is hits[0] (or null if nothing is on PATH at all).
//   - `cursorInstalled` is true when Cursor provides `agent` (a cursor-owned hit exists) OR
//     `cursor-agent` itself is resolvable (Cursor may not have put a bare `agent` on PATH in
//     front, or an unusual layout kept guessOwner() from recognizing its `agent` hit — the
//     'unknown owner shadows a real cursor `agent`' case still needs to count as installed).
//   - `conflict` is true ONLY when Cursor is installed AND the effective `agent` is not
//     cursor-owned (Grok, or an unrecognized binary, is shadowing it). If Cursor's own `agent`
//     wins PATH resolution, or Cursor isn't installed at all, there is nothing to warn about —
//     no matter how many other `agent` launchers exist elsewhere on PATH.
function assessAgentConflict({ env } = {}) {
  const resolvedEnv = env || process.env;
  const hits = detectAgentCollision({ env: resolvedEnv });
  const effective = hits.length ? hits[0] : null;
  const cursorOwnedHit = hits.some(hit => hit.owner === 'cursor');
  const cursorInstalled = cursorOwnedHit || !!findOnPath('cursor-agent', resolvedEnv);
  const conflict = cursorInstalled && !!effective && effective.owner !== 'cursor';
  return { hits, effective, cursorInstalled, conflict };
}

module.exports = {
  ENV_VAR, EXPECTED_NAME, assessAgentConflict, configPath, detectAgentCollision, exeCandidates, loadBinsConfig, resolveBin
};
