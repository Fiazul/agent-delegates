...
async function status(options = {}) {
  ...
  const rows = await Promise.all([Promise.resolve(claudeRow(home)), codexRow(home), agyRow(), grokRow(home, options.probeGrok)]);
  return formatTable(rows);
}

module.exports = { agyRow, claudeRow, codexRow, compactAgyModels, formatTable, grokRow, status };
