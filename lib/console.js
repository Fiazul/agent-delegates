...
async function executeJob(job, directory, mirror) {
  ...
  const cancelTimer = watchForCancel(directory, job.id, () => {
    cancelled = true;
  ...
  const code = await new Promise(resolve => {
    ...
    child.on('close', (status, signal) => resolve(cancelled ? 124 : (status == null ? (signal ? 143 : 1) : status)));
  });
...
async function submitJob(name, job, { timeoutMs } = {}) {
  ...
  while (!fs.existsSync(exitFile)) {
    ...
    if (deadline == null && timeoutMs != null) {
      const startedAt = Number(readFileOrNull(startedFile));
      if (Number.isFinite(startedAt)) deadline = startedAt + timeoutMs;
    }
