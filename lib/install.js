...
// deleted, only renamed aside (reversible).
...
async function setupBinsAndAliases(options, home, delegateIds, main, isTTY, promptText, log, preInstallCollisions) {
  ...
  if (cursorInvolved && grokInvolved && detectAgentCollision) {
    ...
    const owners = new Set(collisions.map(c => c.owner));
    if (owners.has('cursor') && owners.has('grok')) {
      const preOwners = new Set((preInstallCollisions || []).map(c => c.owner));
      const preExisted = preOwners.has('cursor') && preOwners.has('grok');
  ...
  if (wantsAliases == null && isTTY && !options.yes && (nextBins.cursor || nextBins.grok)) {
...
async function install(options = {}) {
  ...
  if (options.skipCliInstall) {
    ...
    await (options.setupVendorClis || setupVendorClis)({
      yes: options.yes,
      log,
  ...
  const authCommands = delegateIds.map(id => VENDORS.find(v => v.id === id)?.auth).filter(Boolean);
