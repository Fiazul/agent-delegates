...
// Cursor's `/usage` TUI screen backs onto a Connect-RPC (protobuf) service, but Connect also
// accepts plain JSON over HTTP POST — no protobuf codec needed. Confirmed live 2026-09-17 against
// api2.cursor.sh (the same default the cursor-agent binary falls back to) with a real account
// token: aiserver.v1.DashboardService/GetCurrentPeriodUsage, empty {} request body, returns
// { billingCycleEnd, planUsage: { totalPercentUsed, autoPercentUsed, apiPercentUsed, ... } }.
// totalPercentUsed is the TUI's "Included X% used"; auto/api are the same breakdown shown there.
function postJson(url, body, headers, timeout = 8000) {
  return new Promise((resolve, reject) => {
    let parsed;
...
async function fetchCursorUsage(accessToken, fetcher = postJson) {
  return fetcher(CURSOR_USAGE_URL, {}, { Authorization: `Bearer ${accessToken}` });
}

// Percent *left* (not used) to match every other row's convention (percentLeft above).
function cursorPercentLeft(used) {
  return Number.isFinite(Number(used)) ? `${Math.max(0, Math.round(100 - Number(used)))}%` : '?';
}

function formatCursorQuota(data) {
  const usage = data && data.planUsage;
  if (!usage) return null;
  let quota = `included ${cursorPercentLeft(usage.totalPercentUsed)} left`;
  const end = Number(data.billingCycleEnd);
  if (Number.isFinite(end) && end > 0) {
    const resets = new Date(end).toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' });
    quota += ` · resets ${resets}`;
  }
  quota += ` (auto ${cursorPercentLeft(usage.autoPercentUsed)} · api ${cursorPercentLeft(usage.apiPercentUsed)})`;
  return quota;
}
...
function claudeRow(home) {
  ...
  if (!snapshot) return ['claude', 'no snapshot yet (statusline writes it)', '-'];
...
async function grokRow(home, probe) {
  ...
  return ['grok', 'logged in · quota via --probe-grok', models];
}
...
async function cursorRow(home = homeDir(), options = {}) {
  ...
  try {
    ...
    if (data.isAuthenticated || data.status === 'authenticated') { quota = 'logged in · no quota API'; loggedIn = true; }
  ...
  // Real quota via the same Connect-RPC call the TUI's `/usage` screen makes (see postJson above).
  // Any failure (no token, network, non-2xx, bad JSON) falls back to today's "no quota API"
  // wording — this must never throw and never block the row.
  if (loggedIn) {
    try {
      const auth = readJson(cursorAuthPath(home));
      const token = auth && auth.accessToken;
      if (token) {
        const usage = await fetchCursorUsage(token, options.fetcher);
        const formatted = formatCursorQuota(usage);
        if (formatted) quota = formatted;
      }
    } catch { /* keep the "no quota API" fallback */ }
  }
...
async function opencodeRow() {
  ...
  let quota = 'logged in · no quota API';
...
async function status(options = {}) {
  ...
  const rows = await Promise.all([
    ...
    cursorRow(home, { fetcher: options.cursorFetcher }),
    opencodeRow()
  ]);
  ...
  if (rows[0] && rows[0][1] === 'no snapshot yet (statusline writes it)') {
...
async function preflight(vendor, home, options = {}) {
  ...
  if (vendor === 'agy') {
    // agy has no auth/status subcommand and no local login marker, but `agy -p /usage` (the same
    // read-only call agyRow uses for the quota table) only answers with usage groups when the CLI
    // is logged in. A login/auth error in its output means logged out; anything else (timeout,
    // missing binary, unparseable) is honestly unknown.
    const bin = resolveBin('agy', { home });
    const runner = options.agyUsage || (() => run(bin.command || 'agy', ['--output-format', 'json', '-p', '/usage'], { timeout: 30000 }));
    let result;
    try { result = await runner(); } catch { result = null; }
    if (result) {
      const combined = `${result.stdout || ''}\n${result.stderr || ''}`;
      try {
        const lines = String(result.stdout || '').trim().split(/\r?\n/);
        const data = JSON.parse(lines.at(-1));
        if (Array.isArray(data?.command?.data?.groups)) {
          return { ok: true, state: 'ok', detail: 'agy -p /usage returned usage groups (logged in)' };
        }
      } catch {}
      if (/not logged in|unauthenticated|unauthorized|please (log|sign) in|login required|authentication required/i.test(combined)) {
        return {
          ok: true, warning: `agy: not logged in — run '${PREFLIGHT_AUTH_COMMAND.agy}' if a job fails with an auth error`,
          state: 'logged-out', detail: 'agy -p /usage reported an authentication error', loginCommand: PREFLIGHT_AUTH_COMMAND.agy
        };
      }
    }
    return {
      ok: true, skipped: true, state: 'unknown',
      detail: 'agy -p /usage gave no usable answer', loginCommand: PREFLIGHT_AUTH_COMMAND.agy
    };
  }
...
module.exports = { agyRow, claudeRow, codexRow, compactAgyModels, compactModelIds, cursorRow, fetchCursorUsage, formatCursorQuota, formatTable, grokRow, opencodeRow, preflight, status };
