...
const SKILLS = ['delegate-codex', 'delegate-antigravity', 'delegate-grok', 'delegate-claude', 'delegates'];
...
function install(options = {}) {
  ...
  const targets = [path.join(home, '.claude', 'skills'), path.join(home, '.agents', 'skills')];
  const ts = timestamp();
  ...
  log('done. try: agent-delegates status');
}
