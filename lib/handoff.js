...
const { CRITICAL_TIER, DEFAULT_TIER, canonicalVendor } = require('./models');
...
async function handoff(jobDir, targetVendor, tier, options = {}) {
  ...
  const targetCanon = canonicalVendor(targetVendor);
  const critical = Boolean(meta?.critical || restOptions.critical);
  let chosenTier = tier;
  // critical:true must reach invoke() on every branch below — including when the caller (or
  // the CRITICAL_TIER lookup) already supplied an explicit tier — so the guard's enforce()
  // still sees this as critical work downstream (e.g. on a later hop) instead of silently
  // losing the flag because a tier happened to be present.
  let invokeOptions = critical ? { ...restOptions, critical: true } : restOptions;
  if (!chosenTier && critical) {
    const criticalTier = CRITICAL_TIER[targetCanon];
    if (!criticalTier && !restOptions.allowSmall) {
      throw new Error(
        `handoff: source job was critical (${meta?.critical ? 'meta.json' : '--critical'}) and ${targetVendor} has no large-model tier — pass --allow-small to override (not recommended) or hand off to a vendor with one (codex sol/astra, agy opus, claude opus, grok best).`
      );
    }
    chosenTier = criticalTier || DEFAULT_TIER[targetCanon];
  } else if (!chosenTier) {
    chosenTier = DEFAULT_TIER[targetCanon];
  }
  const invokeFn = invokeOverride || runner.invoke;
  const result = await invokeFn('run', targetVendor, chosenTier, '-', { ...invokeOptions, cd: cwd }, brief);
